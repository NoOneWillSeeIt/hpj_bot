...
class Questions(StrEnum):
    ...
    EyeDellusion = (
        "В течение часа до начала головной боли отмечал(а) ли ты зрительные нарушения?"
        " (вспышки света, линии-зигзаги, слепые пятна и др.)"
    )
    WhereHurts = "Где отмечалась головная боль?"
    Nature = "Характер головной боли"
    PhysicalTrigger = (
        "Ухудшалась ли головная боль при физической активности?"
    ...
    Painkillers = (
        "Принимал(а) ли ты сегодня препараты от головной боли? "
...
def build_questions() -> dict[Questions, IQuestion]:
    ...
    questions_description = {
        Q.Date: CQ(
            text=Q.Date,
            next_q=Q.HadPain,
            validations=[(validate_date, "Формат ввода: дд.мм")],
            options=suggest_date(),
        ...
        Q.EyeDellusion: CQ(
            text=Q.EyeDellusion, next_q=Q.WhereHurts, options=yes_no_options
        ),
        Q.WhereHurts: CQ(text=Q.WhereHurts, next_q=Q.Nature),
        Q.Nature: CQ(
            text=Q.Nature,
            next_q=Q.PhysicalTrigger,
            options=["Пульсирующая", "Сжимающая"],
        ),
        Q.PhysicalTrigger: CQ(
            text=Q.PhysicalTrigger, next_q=Q.PainIntensity, options=yes_no_options
        ),
        Q.PainIntensity: CQ(
            text=Q.PainIntensity,
            next_q=Q.HadSickness,
            validations=[(validate_pain_intensity, "Нужно ввести число от 1 до 10")],
        ...
        Q.LightIrritation: CQ(
            text=Q.LightIrritation, next_q=Q.SoundIrritation, options=yes_no_options
        ),
        Q.SoundIrritation: CQ(
            text=Q.SoundIrritation, next_q=Q.Causes, options=yes_no_options
        ),
        Q.Causes: CQ(text=Q.Causes, next_q=Q.Painkillers),
        Q.Painkillers: CQ(
            text=Q.Painkillers, next_q=Q.Comments, options=["Нет"], strict_options=False
        ),
        Q.Comments: CQ(
            text=Q.Comments,
            next_q=IQuestion.END,
...
def prepare_answers_for_db(replies: dict) -> Tuple[str, dict]:
    ...
    return prepare_date(replies[Questions.Date]), result
