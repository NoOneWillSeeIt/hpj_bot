...
from tg_bot.constants import DAYS_TO_STORE_BACKUP
...
def delete_marked_entries(db_path: str, chat_id: str | int) -> None:
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            '''
...
def read_entries_keys(db_path: str, chat_id: str | int) -> Set[str]:
...
def mark_entries_for_delete(db_path: str, chat_id: str | int, keys: List[str]) -> None:
